...
const UpdateQuizQA = () => {
  ...
  const handleSubmitQuestionAnswerForQuiz = async () => {
    ...
    // convert image from File Object to Base64
    const questionsClone = _.cloneDeep(questions);
    ...
    const res = await postUpsertWithQA(payload);
    if (res && res.EC === 0) {
  ...
  const fetchQuizWithQA = async () => {
    ...
    if (res && res.EC === 0) {
      const newQA = [];
      // convert image from Base64 to File Object
      for (const question of res.DT.qa) {
        const q = question;
        if (q.imageFile) {
